...
window.Views.dashboard = (d) => {
    ...
    const revenueText = (d.revenueValue || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    ...
    <div class="flex-between mb-4 card glass" style="padding: 10px 20px; flex-wrap: wrap;">
        ...
        <div style="display:flex; gap:10px; flex-wrap: wrap;">
            <select id="dash-filter-status" class="btn btn-secondary btn-sm" onchange="app.filterDashboard(document.getElementById('dash-filter-q').value, this.value, document.getElementById('dash-filter-seller').value, document.getElementById('dash-filter-from').value, document.getElementById('dash-filter-to').value)">
            ...
            <select id="dash-filter-seller" class="btn btn-secondary btn-sm" onchange="app.filterDashboard(document.getElementById('dash-filter-q').value, document.getElementById('dash-filter-status').value, this.value, document.getElementById('dash-filter-from').value, document.getElementById('dash-filter-to').value)">
                <option value="">Vendedor: Todos</option>
                ${sellerOptions}
            </select>
            <input type="date" id="dash-filter-from" class="btn btn-secondary btn-sm" value="${f.from || ''}" 
                   onchange="app.filterDashboard(document.getElementById('dash-filter-q').value, document.getElementById('dash-filter-status').value, document.getElementById('dash-filter-seller').value, this.value, document.getElementById('dash-filter-to').value)">
            <input type="date" id="dash-filter-to" class="btn btn-secondary btn-sm" value="${f.to || ''}" 
                   onchange="app.filterDashboard(document.getElementById('dash-filter-q').value, document.getElementById('dash-filter-status').value, document.getElementById('dash-filter-seller').value, document.getElementById('dash-filter-from').value, this.value)">
            <input type="text" id="dash-filter-q" placeholder="Buscar cliente o No..." class="btn btn-secondary btn-sm" value="${f.query || ''}" 
                   oninput="app.filterDashboard(this.value, document.getElementById('dash-filter-status').value, document.getElementById('dash-filter-seller').value, document.getElementById('dash-filter-from').value, document.getElementById('dash-filter-to').value)"
                   style="width: 200px; text-align: left; cursor: text;">
        </div>
    </div>
    ...
    <div class="stats-grid">
        ...
        <div class="card stat-card" style="min-width: 200px;">
            <div style="display:flex; flex-direction:column; align-items:flex-end; gap:4px;">
                <div class="stat-value" style="font-size: 1.2rem; color: var(--primary-color);">L. ${(d.revenueLPS || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                <div class="stat-value" style="font-size: 1.2rem; color: #3b82f6;">$ ${(d.revenueUSD || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
            </div>
    ...
    <div style="margin-bottom: 2rem;">
        <div class="card glass">
            <h3 style="margin-bottom:1rem;">Top 10 Productos más Cotizados (Cant. y Valor)</h3>
            <div style="height: 400px; width: 100%;"><canvas id="productPerformanceChart"></canvas></div>
        </div>
    </div>
